...
async def handle_scores(lab_name: str = None) -> str:
    ...
    # Find lab using client's built-in search (no regex)
    lab = await client.find_lab_by_name(lab_name)
    if not lab:
        return f"❌ Лабораторная '{lab_name}' не найдена. Используйте /labs для списка доступных."
    
    # Construct lab ID from lab name
    ...
    if not lab_id.startswith("lab-"):
        ...
        for i in range(1, 10):
            if f"Lab {i}" in title or f"Lab 0{i}" in title:
                lab_id = f"lab-{i:02d}"
                break
        else:
    ...
    if not pass_rates:
        # Try alternative lab ID formats
